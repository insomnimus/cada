...
use cada::keys::generate_mappings;
use cada::sound::Note;
	...
	T: cpal::Sample,
{
	...
	let keys = generate_mappings();
	let notes: Vec<Arc<Note>> = vec![];
	for (_, n) in &keys {
		notes.push(Arc::clone(n));
	}

	let next_value = move || notes.iter().map(|x| x.next()).sum();
...
fn listen_keys(keys: HashMap<Key, Arc<Note>>) {
	let callback = move |e: Event| {
		match e {
			EventType::KeyPress(k) => {
				if let Some(p) = keys.get(k) {
					p.play();
				}
			}
